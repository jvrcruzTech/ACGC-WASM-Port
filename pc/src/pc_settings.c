...
#ifdef __EMSCRIPTEN__
EM_ASYNC_JS(char*, pc_web_save_get_text_js, (const char* key_ptr), {
    ...
    try {
        ...
        console.error("[Settings] Failed to load " + key + " from server", err);
        return 0;
    }
    if (response.status === 404) return 0;
    if (!response.ok) {
        console.error("[Settings] Failed to load " + key + " from server: " + response.status);
        return 0;
    }
...
EM_ASYNC_JS(void, pc_web_save_put_text_js, (const char* key_ptr, const char* text_ptr), {
    const key = UTF8ToString(key_ptr);
    ...
    try {
        ...
        console.error("[Settings] Failed to check " + key + " on server", err);
        return;
    }
    ...
    if (!response.ok) {
        console.error("[Settings] Failed to save " + key + " to server: " + response.status);
    }
});
#endif
